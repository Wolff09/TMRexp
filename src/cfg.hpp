	...
	struct Cfg {
		...
		Cfg(std::array<const Statement*, 3> pc, MultiState linstate, const State& obsstateprototype, Shape* shape/*, MultiInOut inout*/)
		  : pc(pc), state(linstate), /*inout(inout),*/ shape(shape),
		    ...
		    valid(false, shape->offset_locals(0), 2*shape->sizeLocals()),
		...
		Cfg(const Cfg& cfg, Shape* shape) : pc(cfg.pc), state(cfg.state), inout(cfg.inout), oracle(cfg.oracle), shape(shape), seen(cfg.seen), own(cfg.own), valid(cfg.valid), smrstate(cfg.smrstate) {}
		Cfg copy() const;
	};
