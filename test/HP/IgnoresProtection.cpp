...
static std::unique_ptr<Program> mk_program() {
	...
	auto retire = Sqz(
		...
		IfThen(
			...
			Sqz(
				...
				SetAssign(2, 0),
				// SetMinus(2, 1), // ignores the collected protected pointers, thus provokes protected pointers to be freed
				Free(2),
				SetMinus(0, 2),
				Clear(1),
				Clear(2),
	...
	std::string name = "HazardPointerImpl_IgnoresProtection";
